...
func NewVolumeAgent(healthCheckBaseDir, socketFile string, healthCheckInterval int, cattleClient cattle.CattleInterface, storagepoolUuid string) *VolumeAgent {
	return &VolumeAgent{
...
func (v *VolumeAgent) Run(controlChan chan bool) error {
	...
	go writeHealthCheckFile(v.healthCheckBaseDir, v.healthCheckInterval, controlChan)

	vols, err := convoy.GetCurrVolumes()
	if err != nil {
		return err
	}
	...
	for _, vol := range vols {
		for {
			...
			if err != nil {
				log.Errorf("Error syncing initial state with rancher server [%v]", err)
				continue
			}
			break
		}
